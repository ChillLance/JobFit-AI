...
describe('memoryProfileRepository (DEMO_MODE)', () => {
  it('getActiveProfile falls back to the default profile before any write', () => {
    const repo = createMemoryProfileRepository()
    expect(repo.readStore()).toBeNull()
    expect(repo.getActiveProfile().id).toBe(defaultJapanCareerProfile.id)
  })

  it('writeStore + getActiveProfile resolves the active profile from memory', () => {
    const repo = createMemoryProfileRepository()
  ...
  it('writeStore overwrites a previous write (upsert, not append)', () => {
    const repo = createMemoryProfileRepository()
